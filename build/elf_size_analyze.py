...
https://github.com/zephyrproject-rtos/zephyr (scripts/footprint/size_report).
    ...
    parser.add_argument("elf", metavar="ELF_FILE", help="path to the examined ELF file")
...
sections must have ALLOC flag and: for RAM - have WRITE flag, for ROM - not have NOBITS type.
    ...
    memory_group.add_argument("-R", "--ram", action="store_true", help="print RAM statistics")
    memory_group.add_argument(
        "-F", "--rom", action="store_true", help='print ROM statistics ("Flash")'
    )
    memory_group.add_argument(
        "-P",
        "--print-sections",
    ...
    printing_group.add_argument(
        "--no-demangle", action="store_true", help="disable demangling of C++ symbol names"
    )
    printing_group.add_argument(
        "--no-merge-paths", action="store_true", help="disable merging paths in the table"
    )
    printing_group.add_argument("--no-color", action="store_true", help="disable colored output")
    printing_group.add_argument(
        "--no-cumulative-size",
        action="store_true",
    ...
    printing_group.add_argument(
        "--no-totals", action="store_true", help="disable printing the total symbols size"
    )
...
class TreeNode:
    ...
    class PreOrderIterator(TreeIterator):
        ...
        def __iter__(self):
            ...
            children_iters = map(lambda child: self.__class__(child, self.depth + 1), self.root)
...
class Symbol:
    ...
    def __init__(
        self, num, name, value, size, type, bind, visibility, section, file=None, line=None
    ):
        self.num = num
        self.name = name
        self.value = value
    ...
    def from_readelf_line(
        cls, line, ignored_types=["NOTYPE", "SECTION", "FILE"], ignore_zero_size=True
    ):
        """
    ...
    def extract_elf_symbols_info(cls, elf_file, readelf_exe=READELF):
        ...
        readelf_proc = subprocess.Popen(
            [readelf_exe, *flags, elf_file], stdout=subprocess.PIPE, universal_newlines=True
        )
        ...
        if readelf_proc.wait(3) != 0:
            raise subprocess.CalledProcessError(readelf_proc.returncode, readelf_proc.args)
...
def extract_elf_symbols_fileinfo(elf_file, nm_exe="nm"):
    ...
    nm_proc = subprocess.Popen([nm_exe, *flags, elf_file], stdout=subprocess.PIPE)
...
def demangle_symbol_names(symbols, cppfilt_exe="c++filt"):
    ...
    if cppfilt_proc.wait(3) != 0:
        raise subprocess.CalledProcessError(cppfilt_proc.returncode, cppfilt_proc.args)
...
class Section:
    ...
    def occupies_rom(self):
        return self.occupies_memory() and self.type not in ["NOBITS"]
...
class SymbolsTreeByPath:
    ...
    def add(self, symbol):
        ...
        if symbol.file is None:
            ...
            if not os.path.isabs(symbol.file):
                log.warning("Symbol's path is not absolute: %s: %s" % (symbol, symbol.file))
    ...
    def sort(self, key, reverse=False):
        ...
        for node in nodes_with_children:
            ...
            path_key = lambda node: (
                -node.cumulative_size if node.cumulative_size is not None else node.data
            )
            ...
            non_symbols = sorted(dirs, key=path_key) + sorted(files, key=path_key) + list(others)
    ...
    def generate_printable_lines(
        ...
        *,
        ...
        field_sizes = self._calculate_field_sizes(
            protolines, max_width=max_width, initial=[len(h) for h in table_headers]
        )
    ...
    def _create_totals_protolines(self, fmt, sizes_dict, human_readable):
        totals = fmt.format(
            "Symbols total", self._size_string(self.total_size, human_readable), "", **sizes_dict
        )
...
def main():
    ...
    def get_exe(name):
        ...
        if "Windows" == platform.system():
            cmd = cmd + ".exe"
        assert shutil.which(cmd) is not None, 'Executable "%s" could not be found!' % cmd
    ...
    def filter_symbols(section_key):
        ...
        filtered = filter(
            lambda symbol: section_key(sections_dict.get(symbol.section, None)), symbols
        )
